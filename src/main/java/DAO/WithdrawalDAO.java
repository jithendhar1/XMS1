	...
	//WithdrawalID, userID, AccountID, WithdrawalDate, Amount
	 public static List<WithdrawalBean> getFilteredWithdrawals(String whereClause, int start, int limit) {
	  ...
	  public static int totalCount() {
			 ...
			 try {
				 connection = DBUtil.provideConnection();
				 String query = "SELECT COUNT(*) AS count FROM (SELECT  MAX(status) AS status FROM transaction WHERE Transactiontype='Withdrawal' GROUP BY TransactionID, transactiondate, userid, DATE(transactiondate), amount) AS test;";
	  ...
	  public static List<TransactionBean>getWithdrawalsByUsername(String username) {
		    ...
		    try {
		        ...
		        if ("Admin".equals(username)) {
		        	...
		        	String adminQuery = "SELECT TransactionID, transactiondate, Amount, userID, MAX(status) AS status FROM transaction WHERE Transactiontype='Withdrawal' GROUP BY TransactionID, transactiondate, userid, Transactiontype, DATE(transactiondate), amount ORDER BY transactiondate DESC, amount";
		...
		depositStatement = connection.prepareStatement(adminQuery);
		            ...
		            if (userResultSet.next()) {
		                ...
		                String depositQuery = "SELECT TransactionID, transactiondate, Amount, userID, MAX(status) AS status FROM transaction WHERE Transactiontype='Withdrawal' AND userid= ? GROUP BY TransactionID, transactiondate, userid, Transactiontype, DATE(transactiondate), amount ORDER BY transactiondate DESC ";
	  ...
	  public static int totalCountByUsername(String username) {
		    ...
		    try {
		        ...
		        if (userResultSet.next()) {
		            ...
		            String countQuery = "SELECT COUNT(*) AS count FROM (SELECT TransactionID, transactiondate, Amount, userID, MAX(status) AS status FROM transaction WHERE Transactiontype = 'Withdrawal' AND userid = ? GROUP BY TransactionID, transactiondate, userid, DATE(transactiondate), amount) test;";
