...
public class DepositDAO {
	 ...
	 public static List<DepositBean> getFilteredDeposits(String whereClause, int start, int limit) {
	  ...
	  public static int totalCount() {
			 ...
			 try {
				 connection = DBUtil.provideConnection();
			   String query = "SELECT COUNT(*) AS count FROM (SELECT  MAX(status) AS status FROM transaction WHERE Transactiontype='Deposit' GROUP BY TransactionID, transactiondate, userID, date(transactiondate), amount) test;";
	  ...
	  public static List<TransactionBean> getDepositsByUsername(String username) {
		    ...
		    try {
		        ...
		        if ("Admin".equals(username)) {
		            ...
		            String adminQuery = "SELECT MAX(TransactionID) AS TransactionID, DATE(transactiondate) AS transactiondate, Amount, userID, MAX(status) AS status FROM transaction WHERE Transactiontype = 'Deposit' GROUP BY userID, Transactiontype, DATE(transactiondate), Amount ORDER BY transactiondate DESC";
		            ...
		            if (userResultSet.next()) {
		                ...
		                String depositQuery = "SELECT MAX(TransactionID) AS TransactionID, DATE(transactiondate) AS transactiondate, Amount, userID, MAX(status) AS status FROM transaction WHERE Transactiontype='Deposit' AND userID= ? GROUP BY userID, Transactiontype, DATE(transactiondate), Amount ORDER BY transactiondate DESC";
	  ...
	  public static int totalCountByUsername(String username) {
		    ...
		    try {
		        ...
		        if (userResultSet.next()) {
		            ...
		            String countQuery = "SELECT COUNT(*) AS count FROM (SELECT  MAX(status) AS status FROM transaction WHERE Transactiontype='Deposit' AND userID= ? GROUP BY TransactionID,  userid, date(transactiondate), amount) AS test";
