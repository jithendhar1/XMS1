	 ...
	 public static List<TransactionBean>getWithdrawalsByUsername(String username) {
		    ...
		    try {
		        ...
		        if ("Admin".equals(username)) {
		            ...
		            String adminQuery = "SELECT  UserID, date(transactiondate)as transactiondate, TransactionID,Amount, ReferralID  ,   max(status) status  FROM transaction where Transactiontype='Bonus' group by userid, Transactiontype, date(transactiondate),amount";
		            ...
		            if (userResultSet.next()) {
		                ...
		                String depositQuery = "SELECT  UserID, date(transactiondate) as transactiondate, TransactionID,Amount, ReferralID  ,   max(status) status  FROM transaction where Transactiontype='Bonus' AND userID = ? group by userid, Transactiontype, date(transactiondate),amount ";
