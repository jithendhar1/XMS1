...
public class ROIServiceImpl {
	...
	public String addV(String Approvedby, String ROIAmount, String ModifiedDate, String TransactionID) {
	        ...
	        if (resultSet.next()) {
	            ...
	            if (count > 0) {
	                ...
	                String insertNewRecordQuery = "INSERT INTO transaction (userID, openamount, closingamount, transactiondate, status, Approvedby, Transactiontype, TransactionID, Amount) " +
	                        "SELECT userID, Amount AS openamount, Amount + (Amount * ?)/100 AS closingamount, ?, 1, 'Admin', 'ROI', ?, ? FROM customeraccdetails where Amount > 0";
	                ...
	                try (PreparedStatement insertPs = con.prepareStatement(insertNewRecordQuery)) {
	                    ...
	                    insertPs.setString(4, ROIAmount);
	                    ...
	                    if (rowsAffected > 0) {
	                       ...
	                       String updatecustaccQuery = "update customeraccdetails set Amount = Amount + (Amount * ?)/100";

	                       PreparedStatement updateps = con.prepareStatement(updatecustaccQuery);
	                       updateps.setString(1, ROIAmount);
